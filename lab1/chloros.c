...
static struct thread* thread_new(threadfn_t fn, void* arg) {
    ...
    struct thread* t = aligned_alloc(STACK_ALIGNMENT, sizeof(struct thread));
    if (t == NULL) {
        return NULL;
    }
    ...
    t->stack = (uint8_t*) aligned_alloc(STACK_ALIGNMENT, STACK_SIZE);
    if (t->stack == NULL) {
        return NULL;
    }

    t->ctx.rbp = (uint64_t) t->stack;
    t->ctx.rsp = t->ctx.rbp + STACK_SIZE;
    t->ctx.mxcsr = 0x1F80;
    t->ctx.x87 = 0x037F;

    // Initialize function into rdi and args into rsi
    // Note: check that rbp is where these should be pushed based on
    // thread_start code
    t->ctx.rsp = (uint64_t) ((void**) t->ctx.rsp - 2);
    *(void**) t->ctx.rsp = (void*) thread_start;
    *((void**) t->ctx.rsp + 1) = fn;
    *((void**) t->ctx.rsp + 2) = arg;
    return t;
}
...
bool thread_init(void) {
    ...
    // FIXME: register the initial thread (the currently executing context)
    // as a thread. It just needs a thread object but doesn't need a stack
    // or any initial context as it is already running. Make sure to update
    // 'running' since it is already running.
    running = aligned_alloc(STACK_ALIGNMENT, sizeof(struct thread));
    if (running == NULL) {
        return false;
    }
    running->id = 0;
